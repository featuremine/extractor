 ...
 * @date 22 Aug 2018
 * @brief File contains tests for the "less" logical operator feature
 *
...
TEST(less_comp, multiple_field) {
  ...
  auto *less_param_t = fm_tuple_type_get(tsys, 0);
  ...
  auto *comp_C = fm_comp_decl(sys, g, "less", 2, less_param_t, comp_B, comp_A);
  ASSERT_NE(comp_C, nullptr);

  auto *comp_D =
      fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_C,
                   (src_dir + "/data/less_multiple.test.csv").c_str());
  ...
  EXPECT_BASE((src_dir + "/data/less_multiple.base.csv").c_str(),
              (src_dir + "/data/less_multiple.test.csv").c_str());
}

TEST(less_comp, string_field) {
  using namespace std;
  ...
  cout << "Building less" << endl;
  auto *comp_C = fm_comp_decl(sys, g, "less", 2, less_param_t, comp_B, comp_A);
  ASSERT_NE(comp_C, nullptr);
  cout << "Built less" << endl;

  auto *comp_D = fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_C,
                              (src_dir + "/data/less_string.test.csv").c_str());
  ASSERT_NE(comp_D, nullptr);
  ...
  EXPECT_BASE((src_dir + "/data/less_string.base.csv").c_str(),
              (src_dir + "/data/"
                         "less_string.test.csv")
                  .c_str());
}

TEST(less_comp, single_field) {
  using namespace std;
  ...
  auto *comp_C =
      fm_comp_decl(sys, g, "less", 2, less_param_t, comp_BF, comp_AF);
  ASSERT_NE(comp_C, nullptr);

  auto *comp_D = fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_C,
                              (src_dir + "/data/less_single.test.csv").c_str());
  ASSERT_NE(comp_D, nullptr);
  ...
  EXPECT_BASE((src_dir + "/data/less_single.base.csv").c_str(),
              (src_dir + "/data/"
                         "less_single.test.csv")
                  .c_str());
}

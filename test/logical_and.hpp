 ...
 * @date 22 Aug 2018
 * @brief File contains tests for the "and" logical operator feature
 *
...
TEST(and_comp, multiple_field) {
  ...
  auto *and_param_t = fm_tuple_type_get(tsys, 0);
  ...
  auto *comp_C =
      fm_comp_decl(sys, g, "logical_and", 2, and_param_t, comp_AB, comp_BA);
  ASSERT_NE(comp_C, nullptr);

  auto *comp_D =
      fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_C,
                   (src_dir + "/data/and_multiple.test.csv").c_str());
  ...
  EXPECT_BASE((src_dir + "/data/and_multiple.base.csv").c_str(),
              (src_dir + "/data/"
                         "and_multiple.test.csv")
                  .c_str());
}

TEST(and_comp, single_field) {
  using namespace std;
  ...
  auto *comp_C =
      fm_comp_decl(sys, g, "logical_and", 2, and_param_t, comp_AF, comp_BF);
  ASSERT_NE(comp_C, nullptr);

  auto *comp_D = fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_C,
                              (src_dir + "/data/and_single.test.csv").c_str());
  ASSERT_NE(comp_D, nullptr);
  ...
  EXPECT_BASE((src_dir + "/data/and_single.base.csv").c_str(),
              (src_dir + "/data/"
                         "and_single.test.csv")
                  .c_str());
}

TEST(and_comp, multiple_comps) {
  using namespace std;
  ...
  auto *comp_D = fm_comp_decl(sys, g, "logical_and", 3, and_param_t, comp_BC,
                              comp_BA, comp_CA);
  ...
  auto *comp_E =
      fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_D,
                   (src_dir + "/data/and_multiple_comps.test.csv").c_str());
  ...
  EXPECT_BASE((src_dir + "/data/and_multiple_comps.base.csv").c_str(),
              (src_dir + "/data/and_multiple_comps.test.csv").c_str());
}

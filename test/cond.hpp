 ...
 * @date 22 Aug 2018
 * @brief File contains tests for the "cond" logical operator feature
 *
...
TEST(cond_comp, single_field) {
  ...
  auto *cond_param_t = fm_tuple_type_get(tsys, 0);
  ...
  auto *comp_C =
      fm_comp_decl(sys, g, "greater", 2, cond_param_t, comp_BF, comp_AF);
  ASSERT_NE(comp_C, nullptr);

  auto *comp_CC =
      fm_comp_decl(sys, g, "cond", 3, cond_param_t, comp_C, comp_B, comp_A);
  ASSERT_NE(comp_CC, nullptr);

  auto *comp_D = fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_CC,
                              (src_dir + "/data/cond_single.test.csv").c_str());
  ...
  EXPECT_NUMDIFF_BASE((src_dir + "/data/cond_single.base.csv").c_str(),
                      (src_dir + "/data/"
                                 "cond_single.test.csv")
                          .c_str());
}

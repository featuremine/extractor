 ...
 * @date 23 Aug 2018
 * @brief File contains tests for the "substr" logical operator feature
 *
...
TEST(substr_comp, multiple_field) {
  using namespace std;
  ...
  auto *fields_param_t = fm_tuple_type_get(
      tsys, 1,
      fm_tuple_type_get(tsys, 3, fm_cstring_type_get(tsys),
                        fm_cstring_type_get(tsys), fm_cstring_type_get(tsys)));
  auto *substr_param_t = fm_tuple_type_get(tsys, 1, fm_cstring_type_get(tsys));
  ...
  auto *comp_A = fm_comp_decl(
      ...
      "val2", fm_array_type_get(tsys, fm_base_type_get(tsys, FM_TYPE_CHAR), 16),
      "", "text",
      fm_array_type_get(tsys, fm_base_type_get(tsys, FM_TYPE_CHAR), 16), "");
  ASSERT_NE(comp_A, nullptr);

  auto *comp_AF = fm_comp_decl(sys, g, "fields", 1, fields_param_t, comp_A,
                               "text", "val1", "val2");
  ASSERT_NE(comp_AF, nullptr);

  auto *comp_B = fm_comp_decl(sys, g, "find", 1, substr_param_t, comp_AF, "te");
  ASSERT_NE(comp_B, nullptr);

  auto *comp_C =
      fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_B,
                   (src_dir + "/data/substr_multiple.test.csv").c_str());
  ...
  EXPECT_BASE((src_dir + "/data/substr_multiple.base.csv").c_str(),
              (src_dir + "/data/substr_multiple.test.csv").c_str());
}

TEST(substr_comp, single_field) {
  using namespace std;
  ...
  auto *comp_AF =
      fm_comp_decl(sys, g, "field", 1, field_param_t, comp_A, "text");
  ASSERT_NE(comp_AF, nullptr);
  ...
  auto *comp_C =
      fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_B,
                   (src_dir + "/data/substr_single.test.csv").c_str());
  ASSERT_NE(comp_C, nullptr);
  ...
  EXPECT_BASE((src_dir + "/data/substr_single.base.csv").c_str(),
              (src_dir + "/data/substr_single.test.csv").c_str());
}

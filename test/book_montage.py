...
class ValidationBook:
    ...
    def validate_side(self, validationside, bookside):
        ...
        # TODO: change float(str to decimal once float to decimal is available)
        for price, level in bookside:
            if float(str(price)) not in validationside:
                return False
            validationlevel = validationside[float(str(price))]
            ...
            for order in level:
                if order.id not in validationlevel:
                    return False
                if validationlevel[order.id] != float(str(order.qty)):
                    return False

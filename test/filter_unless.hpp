...
/**
 * @file filter_unless.hpp
 * @authors Andres Rangel
 * @date 22 Aug 2018
 * @brief File contains tests for the "unless" logical operator feature
 *
...
TEST(unless_comp, multiple_comps) {
  using namespace std;
  ...
  auto *comp_C =
      fm_comp_decl(sys, g, "filter_unless", 2, nullptr, comp_AF, comp_B);
  ASSERT_NE(comp_C, nullptr);

  auto *comp_D =
      fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_C,
                   (src_dir + "/data/unless_multiple_comps.test.csv").c_str());
  ...
  EXPECT_NUMDIFF_BASE(
      (src_dir + "/data/unless_multiple_comps.base.csv").c_str(),
      (src_dir + "/data/"
                 "unless_multiple_comps.test.csv")
          .c_str());
}

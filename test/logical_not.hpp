 ...
 * @date 22 Aug 2018
 * @brief File contains tests for the "not" logical operator feature
 *
...
TEST(not_comp, multiple_field) {
  ...
  auto *not_param_t = fm_tuple_type_get(tsys, 0);
  ...
  auto *comp_C = fm_comp_decl(sys, g, "logical_not", 1, not_param_t, comp_BA);
  ASSERT_NE(comp_C, nullptr);

  auto *comp_D =
      fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_C,
                   (src_dir + "/data/not_multiple.test.csv").c_str());
  ...
  EXPECT_BASE((src_dir + "/data/not_multiple.base.csv").c_str(),
              (src_dir + "/data/"
                         "not_multiple.test.csv")
                  .c_str());
}

TEST(not_comp, single_field) {
  using namespace std;
  ...
  auto *comp_C = fm_comp_decl(sys, g, "logical_not", 1, not_param_t, comp_AF);
  ASSERT_NE(comp_C, nullptr);

  auto *comp_D = fm_comp_decl(sys, g, "csv_record", 1, cstr_t, comp_C,
                              (src_dir + "/data/not_single.test.csv").c_str());
  ASSERT_NE(comp_D, nullptr);
  ...
  EXPECT_BASE((src_dir + "/data/not_single.base.csv").c_str(),
              (src_dir + "/data/"
                         "not_single.test.csv")
                  .c_str());
}

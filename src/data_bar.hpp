...
bool fm_comp_data_bar_stream_exec(fm_frame_t *result, size_t args,
  ...
  fmc_time64_t start = comp_cl->period_ * ((now - comp_cl->offset_) / comp_cl->period_) + comp_cl->offset_;
...
fm_ctx_def_t *fm_comp_data_bar_gen(fm_comp_sys_t *csys,
  ...
  if (argc != 1 || fm_type_frame_nfields(argv[0]) != 1 ||
      fm_type_base_enum(fm_type_frame_field_type(argv[0], 0)) != FM_TYPE_TIME64) {

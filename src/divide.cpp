...
struct divide_field_exec {
  virtual ~divide_field_exec() {}
  virtual void exec(fm_frame_t *result, size_t,
                    const fm_frame_t *const argv[]) = 0;
};

template <class T> struct the_divide_field_exec_2_0 : divide_field_exec {
  the_divide_field_exec_2_0(fm_field_t field) : field_(field) {}
...
struct divide_comp_cl {
  ~divide_comp_cl() {
    for (auto *ptr : calls) {
      delete ptr;
    }
  }
  vector<divide_field_exec *> calls;
};

bool fm_comp_divide_call_stream_init(fm_frame_t *result, size_t args,
                                     const fm_frame_t *const argv[],
                                     fm_call_ctx_t *ctx, fm_call_exec_cl *cl) {
  auto &calls = (*(divide_comp_cl *)ctx->comp).calls;
  for (auto &call : calls) {
...
bool fm_comp_divide_stream_exec(fm_frame_t *result, size_t args,
                                const fm_frame_t *const argv[],
                                fm_call_ctx_t *ctx, fm_call_exec_cl cl) {
  auto &calls = (*(divide_comp_cl *)ctx->comp).calls;
  for (auto &call : calls) {
...
fm_call_def *fm_comp_divide_stream_call(fm_comp_def_cl comp_cl,
                                        const fm_ctx_def_cl ctx_cl) {
  auto *def = fm_call_def_new();
  fm_call_def_init_set(def, fm_comp_divide_call_stream_init);
  fm_call_def_exec_set(def, fm_comp_divide_stream_exec);
  return def;
}

template <class... Ts>
divide_field_exec *get_divide_field_exec(fmc::type_list<Ts...>,
                                         fm_type_decl_cp f_type, int idx) {
  divide_field_exec *result = nullptr;
  auto create = [&](auto t) {
    using Tt = decltype(t);
...
fm_ctx_def_t *fm_comp_divide_gen(fm_comp_sys_t *csys, fm_comp_def_cl closure,
  ...
  if (fm_type_frame_nfields(argv[0]) == 1 &&
    ...
    if (!fm_type_equal(type0, type1)) {
      auto *errstr = "the two frames have fields of different types";
      fm_type_sys_err_custom(sys, FM_TYPE_ERROR_ARGS, errstr);
      return nullptr;
    }
  ...
  using supported_types = fmc::type_list<RATIONAL64, FLOAT32, FLOAT64>;

  auto inp = argv[0];
  int nf = fm_type_frame_nfields(inp);
  for (int idx = 0; idx < nf; ++idx) {
    auto f_type = fm_type_frame_field_type(inp, idx);
    auto *call = get_divide_field_exec(supported_types(), f_type, idx);
    if (!call) {
      ostringstream os;
      auto *str = fm_type_to_str(f_type);
      os << "type " << str << "is not supported in divide feature";
      free(str);
  ...
  fm_ctx_def_stream_call_set(def, &fm_comp_divide_stream_call);
  fm_ctx_def_query_call_set(def, nullptr);
  return def;
}

void fm_comp_divide_destroy(fm_comp_def_cl cl, fm_ctx_def_t *def) {
  auto *ctx_cl = (divide_comp_cl *)fm_ctx_def_closure(def);
  if (ctx_cl != nullptr)
    delete ctx_cl;
}

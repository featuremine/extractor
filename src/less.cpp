 ...
 * @date 22 Aug 2018
 * @brief File contains C++ definitions for the "less" logical operator
 *
...
template <class T> struct the_less_field_exec_2_0 : op_field_exec {
  the_less_field_exec_2_0(fm_field_t field) : field_(field) {}
...
template <> struct the_less_field_exec_2_0<char *> : op_field_exec {
  the_less_field_exec_2_0(fm_field_t field) : field_(field) {}
...
bool fm_comp_less_call_stream_init(fm_frame_t *result, size_t args,
                                   const fm_frame_t *const argv[],
                                   fm_call_ctx_t *ctx, fm_call_exec_cl *cl) {
  auto &calls = (*(less_comp_cl *)ctx->comp).calls;
  for (auto &call : calls) {
...
bool fm_comp_less_stream_exec(fm_frame_t *result, size_t args,
                              const fm_frame_t *const argv[],
                              fm_call_ctx_t *ctx, fm_call_exec_cl cl) {
  auto &calls = (*(less_comp_cl *)ctx->comp).calls;
  for (auto &call : calls) {
...
fm_call_def *fm_comp_less_stream_call(fm_comp_def_cl comp_cl,
                                      const fm_ctx_def_cl ctx_cl) {
  auto *def = fm_call_def_new();
  fm_call_def_init_set(def, fm_comp_less_call_stream_init);
  fm_call_def_exec_set(def, fm_comp_less_stream_exec);
  return def;
}

template <class... Ts>
op_field_exec *get_less_field_exec(fmc::type_list<Ts...>,
  ...
  auto create = [&](auto t) {
    ...
    if (!result && obj.validate(f_type)) {
      result = new the_less_field_exec_2_0<Tn>(idx);
    }
  };
...
fm_ctx_def_t *fm_comp_less_gen(fm_comp_sys_t *csys, fm_comp_def_cl closure,
  ...
  auto ctx_cl = make_unique<less_comp_cl>();
  ...
  for (int idx = 0; idx < nf; ++idx) {
    ...
    if (fm_type_is_base(f_type)) {
      auto *call = get_less_field_exec(supported_types(), f_type, idx);
      if (!call) {
        ostringstream os;
        auto *str = fm_type_to_str(f_type);
        os << "type " << str << "is not supported in less feature";
        free(str);
      ...
      if (fm_type_is_base(af_type)) {
        ...
        case FM_TYPE_CHAR:
          calls.push_back(new the_less_field_exec_2_0<char *>(idx));
          break;
        default:
          ostringstream os;
          auto *str = fm_type_to_str(af_type);
          os << "array of type " << str
             << "is not supported in less "
                "feature";
          free(str);
          fm_type_sys_err_custom(sys, FM_TYPE_ERROR_ARGS, os.str().c_str());
          return nullptr;
  ...
  fm_ctx_def_stream_call_set(def, &fm_comp_less_stream_call);
  fm_ctx_def_query_call_set(def, nullptr);
  return def;
}

void fm_comp_less_destroy(fm_comp_def_cl cl, fm_ctx_def_t *def) {
  auto *ctx_cl = (less_comp_cl *)fm_ctx_def_closure(def);
  if (ctx_cl != nullptr)
    delete ctx_cl;
}

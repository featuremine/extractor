...
template <class T> struct max_exec_cl : public exec_cl {
  max_exec_cl(fm_field_t field) : field_(field) {}

  bool exec(fm_frame_t *result, size_t, const fm_frame_t *const argv[]) {
    T val = *(const T *)fm_frame_get_cptr1(argv[0], field_, 0);

    if (!(val <= curr_max_)) {
      curr_max_ = *(T *)fm_frame_get_ptr1(result, field_, 0) = val;
      return true;
    }
    return false;
  }
...
template <> struct max_exec_cl<double> : public exec_cl {
  max_exec_cl(fm_field_t field) : field_(field) {}
  ...
  bool exec(fm_frame_t *result, size_t, const fm_frame_t *const argv[]) {
    ...
    if (!(val <= curr_max_)) {
      curr_max_ = *(double *)fm_frame_get_ptr1(result, field_, 0) = val;
      return true;
    }
    return false;
  }
...
template <> struct max_exec_cl<float> : public exec_cl {
  max_exec_cl(fm_field_t field) : field_(field) {}
  ...
  bool exec(fm_frame_t *result, size_t, const fm_frame_t *const argv[]) {
    ...
    if (!(val <= curr_max_)) {
      curr_max_ = *(float *)fm_frame_get_ptr1(result, field_, 0) = val;
      return true;
    }
    return false;
  }
...
bool fm_comp_max_call_stream_init(fm_frame_t *result, size_t args,
                                  const fm_frame_t *const argv[],
                                  fm_call_ctx_t *ctx, fm_call_exec_cl *cl) {
  auto *closure = (max_comp_cl *)ctx->comp;
  for (auto *call : closure->calls)
    call->reset(result, args, argv);
  return true;
}

bool fm_comp_max_stream_exec(fm_frame_t *result, size_t args,
...
fm_call_def *fm_comp_max_stream_call(fm_comp_def_cl comp_cl,
                                     const fm_ctx_def_cl ctx_cl) {
  auto *def = fm_call_def_new();
  fm_call_def_init_set(def, fm_comp_max_call_stream_init);
  fm_call_def_exec_set(def, fm_comp_max_stream_exec);
  return def;
}

void fm_comp_max_queuer(size_t idx, fm_call_ctx_t *ctx) {
  int64_t &value = ((max_comp_cl *)(ctx->comp))->t;
  fmc_time64_t prev;
  prev.value = value;
  auto now = fm_stream_ctx_now((fm_stream_ctx_t *)ctx->exec);
...
template <class... Ts>
exec_cl *get_comp_max_cl(fmc::type_list<Ts...>, fm_type_decl_cp f_type,
                         int idx) {
...
fm_ctx_def_t *fm_comp_max_gen(fm_comp_sys_t *csys, fm_comp_def_cl closure,
  ...
  auto ctx_cl = make_unique<max_comp_cl>();
  ...
  for (int idx = 0; idx < nf; ++idx) {
    ...
    auto *call = get_comp_max_cl(supported_types(), f_type, idx);
    if (call == nullptr) {
      ostringstream os;
  ...
  fm_ctx_def_queuer_set(def, &fm_comp_max_queuer);
  fm_ctx_def_stream_call_set(def, &fm_comp_max_stream_call);
  fm_ctx_def_query_call_set(def, nullptr);

  return def;
}

void fm_comp_max_destroy(fm_comp_def_cl cl, fm_ctx_def_t *def) {
  auto *ctx_cl = (max_comp_cl *)fm_ctx_def_closure(def);
  if (ctx_cl != nullptr)
    delete ctx_cl;
}

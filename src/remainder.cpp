...
struct remainder_field_exec {
  virtual ~remainder_field_exec() {}
  virtual void exec(fm_frame_t *result, size_t,
                    const fm_frame_t *const argv[]) = 0;
};

template <class T> struct the_remainder_field_exec_2_0 : remainder_field_exec {
  the_remainder_field_exec_2_0(fm_field_t field) : field_(field) {}
...
struct remainder_comp_cl {
  ~remainder_comp_cl() {
    for (auto *ptr : calls) {
      delete ptr;
    }
  }
  vector<remainder_field_exec *> calls;
};
...
remainder_field_exec *get_remainder_field_exec(fmc::type_list<Ts...>,
  ...
  auto create = [&](auto t) {
    ...
    if (!result && obj.validate(f_type)) {
      ...
      result = new the_remainder_field_exec_2_0<S>(idx);
    }
  };
...
fm_ctx_def_t *fm_comp_remainder_gen(fm_comp_sys_t *csys, fm_comp_def_cl closure,
  ...
  auto ctx_cl = make_unique<remainder_comp_cl>();
  auto &calls = ctx_cl->calls;

  using supported_types = fmc::type_list<INT64>;

  auto inp = argv[0];
  int nf = fm_type_frame_nfields(inp);
  for (int idx = 0; idx < nf; ++idx) {
    auto f_type = fm_type_frame_field_type(inp, idx);
    auto *call = get_remainder_field_exec(supported_types(), f_type, idx);
    if (!call) {
      ostringstream os;
      auto *str = fm_type_to_str(f_type);
      os << "type " << str << "is not supported in remainder feature";
      free(str);
  ...
  fm_ctx_def_stream_call_set(def, &fm_comp_remainder_stream_call);
  fm_ctx_def_query_call_set(def, nullptr);
  return def;
}

void fm_comp_remainder_destroy(fm_comp_def_cl cl, fm_ctx_def_t *def) {
  auto *ctx_cl = (remainder_comp_cl *)fm_ctx_def_closure(def);
  if (ctx_cl != nullptr)
    delete ctx_cl;
}

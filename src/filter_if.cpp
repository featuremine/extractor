 ...
 * @date 22 Aug 2018
 * @brief File contains C++ definitions for the "filter_if" logical operator
 *
...
struct the_filter_if_field_exec_2_0 : op_field_exec {
  the_filter_if_field_exec_2_0(fm_field_t field) : field_(field) {}
...
bool fm_comp_filter_if_stream_exec(fm_frame_t *result, size_t args,
                                   const fm_frame_t *const argv[],
...
fm_call_def *fm_comp_filter_if_stream_call(fm_comp_def_cl comp_cl,
                                           const fm_ctx_def_cl ctx_cl) {
  auto *def = fm_call_def_new();
  fm_call_def_init_set(def, fm_comp_filter_if_call_stream_init);
  fm_call_def_exec_set(def, fm_comp_filter_if_stream_exec);
  return def;
}
...
fm_ctx_def_t *fm_comp_filter_if_gen(fm_comp_sys_t *csys, fm_comp_def_cl closure,
  ...
  for (size_t idx = 0; idx < nf; ++idx) {
    ...
    auto *call = new the_filter_if_field_exec_2_0(idx);
    if (!call) {
      ostringstream os;
  ...
  fm_ctx_def_queuer_set(def, &fm_comp_filter_if_queuer);
  fm_ctx_def_stream_call_set(def, &fm_comp_filter_if_stream_call);
  fm_ctx_def_query_call_set(def, nullptr);
  return def;
}

void fm_comp_filter_if_destroy(fm_comp_def_cl cl, fm_ctx_def_t *def) {
  auto *ctx_cl = (filter_if_comp_cl *)fm_ctx_def_closure(def);
  if (ctx_cl != nullptr)
    delete ctx_cl;
}

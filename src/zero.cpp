...
bool fm_comp_zero_call_stream_init(fm_frame_t *result, size_t args,
                                   const fm_frame_t *const argv[],
                                   fm_call_ctx_t *ctx, fm_call_exec_cl *cl) {
  auto nf = fm_type_frame_nfields(fm_frame_type(result));
  for (unsigned i = 0; i < nf; ++i) {
    *(double *)fm_frame_get_ptr1(result, i, 0) = 0.0;
  }
  return true;
}

bool fm_comp_zero_stream_exec(fm_frame_t *result, size_t,
                              const fm_frame_t *const argv[],
                              fm_call_ctx_t *ctx, fm_call_exec_cl cl) {
  return false;
}

fm_call_def *fm_comp_zero_stream_call(fm_comp_def_cl comp_cl,
                                      const fm_ctx_def_cl ctx_cl) {
  auto *def = fm_call_def_new();
  fm_call_def_init_set(def, fm_comp_zero_call_stream_init);
  fm_call_def_exec_set(def, fm_comp_zero_stream_exec);
  return def;
}

fm_ctx_def_t *fm_comp_zero_gen(fm_comp_sys_t *csys, fm_comp_def_cl closure,
  ...
  fm_ctx_def_stream_call_set(def, &fm_comp_zero_stream_call);

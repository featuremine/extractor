...
bool fm_comp_book_build_call_stream_init(fm_frame_t *result, size_t args,
  ...
  for (unsigned i = 0; i < exe_cl->lvl_cnt; ++i) {
    ...
    *(fmc_fxpt128_t *)fm_frame_get_ptr1(result, fields.back(), 0) =
        fmc::fxpt128();
...
bool fm_comp_book_build_stream_exec(fm_frame_t *result, size_t args,
  ...
  //                     << msg.seqn << ','
  //                     << msg.id << ','
  //                     << fmc_fxpt128_to_double(msg.price) << ','
  //                     << msg.qty << ','
  //                     << msg.is_bid << ','
  //                     << msg.batch << endl;
  //            },
  ...
  //                     << msg.id << ','
  //                     << fmc_fxpt128_to_double(msg.price) << ','
  //                     << fmc_fxpt128_to_double(msg.trade_price) << ','
  //                     << msg.qty << ','
  //                     << msg.is_bid << ','
  //                     << msg.batch << endl;
  //            },
...
fm_ctx_def_t *fm_comp_book_build_gen(fm_comp_sys_t *csys,
  ...
  for (unsigned i = 0; i < lvl_cnt; ++i) {
    types[idx] = fm_base_type_get(sys, FM_TYPE_FIXEDPOINT128);

...
struct diff_field_exec {
  virtual ~diff_field_exec() {}
  virtual void exec(fm_frame_t *result, size_t,
                    const fm_frame_t *const argv[]) = 0;
};

template <class T> struct the_diff_field_exec_2_0 : diff_field_exec {
  the_diff_field_exec_2_0(fm_field_t field) : field_(field) {}
...
struct diff_comp_cl {
  ...
  vector<diff_field_exec *> calls;
};

bool fm_comp_diff_call_stream_init(fm_frame_t *result, size_t args,
                                   const fm_frame_t *const argv[],
                                   fm_call_ctx_t *ctx, fm_call_exec_cl *cl) {
  auto &calls = (*(diff_comp_cl *)ctx->comp).calls;
  for (auto &call : calls) {
...
bool fm_comp_diff_stream_exec(fm_frame_t *result, size_t args,
                              const fm_frame_t *const argv[],
                              fm_call_ctx_t *ctx, fm_call_exec_cl cl) {
  auto &calls = (*(diff_comp_cl *)ctx->comp).calls;
  for (auto &call : calls) {
...
fm_call_def *fm_comp_diff_stream_call(fm_comp_def_cl comp_cl,
                                      const fm_ctx_def_cl ctx_cl) {
  auto *def = fm_call_def_new();
  fm_call_def_init_set(def, fm_comp_diff_call_stream_init);
  fm_call_def_exec_set(def, fm_comp_diff_stream_exec);
  return def;
}

template <class... Ts>
diff_field_exec *get_diff_field_exec(fmc::type_list<Ts...>,
                                     fm_type_decl_cp f_type, int idx) {
  diff_field_exec *result = nullptr;
  auto create = [&](auto t) {
    using Tt = decltype(t);
...
fm_ctx_def_t *fm_comp_diff_gen(fm_comp_sys_t *csys, fm_comp_def_cl closure,
  ...
  auto ctx_cl = make_unique<diff_comp_cl>();
  ...
  for (int idx = 0; idx < nf; ++idx) {
    ...
    auto *call = get_diff_field_exec(supported_types(), f_type, idx);
    if (!call) {
      ostringstream os;
      auto *str = fm_type_to_str(f_type);
      os << "type " << str << "is not supported in diff feature";
      free(str);
  ...
  fm_ctx_def_stream_call_set(def, &fm_comp_diff_stream_call);
  fm_ctx_def_query_call_set(def, nullptr);
  return def;
}

void fm_comp_diff_destroy(fm_comp_def_cl cl, fm_ctx_def_t *def) {
  auto *ctx_cl = (diff_comp_cl *)fm_ctx_def_closure(def);
  if (ctx_cl != nullptr)
    delete ctx_cl;
}

...
struct the_round_field_exec_2_0<int64_t, fmc_decimal128_t> : round_field_exec {
  ...
  void exec(fm_frame_t *result, size_t,
    ...
    fmc_error_t *err;
    fmc_decimal128_round(&ret, &val0, &err);
...
fm_ctx_def_t *fm_comp_round_gen(fm_comp_sys_t *csys, fm_comp_def_cl closure,
  ...
  if (fm_type_tuple_size(ptype) == 1 || fm_type_tuple_size(ptype) == 2) {
    ...
    if (fm_type_tuple_size(ptype) == 2) {
      ...
      if (!restype)
        return error();
      is64 = fm_type_equal(restype, fm_base_type_get(sys, FM_TYPE_DECIMAL64));
    ...
    if (is64 && DECIMAL64_FRACTION % divisor != 0) {
    ...
    auto decimal_param_t = is64 ? fm_base_type_get(sys, FM_TYPE_DECIMAL64)
    ...
    for (int idx = 0; idx < nf; ++idx) {
      ...
      round_field_exec *call = is64
        ? get_round_field_exec<fm_decimal64_t>(supported_types(), f_type, idx, divisor)
